...
const VenteForm:FunctionComponent<BtnProps> = ({open,data,loading,setOpen,handleValidate}) => {
  ...
  const handleChoose=(val:any)=>{
    ...
    setPanierProduit([{produit_id:val.id,remise:0,qte:1,quantite:1,prix_vente:val.pv,pv:val.pv,total:val.pv,designation:val.designation},...panierProduit])
    calculTotal([{produit_id:val.id,remise:0,qte:1,quantite:1,prix_vente:val.pv,pv:val.pv,total:val.pv,designation:val.designation},...panierProduit])
    setProduitVente([])
    setSearch("")
  }
                    ...
                    >
                        <Dialog.Panel style={{borderWidth:2,borderColor:"#ff8041"}} className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-8xl text-black dark:text-white-dark">
                            ...
                            <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                 <div style={{color:"#ff8041"}} className="uppercase underline">{'Vente produits' }</div>
                            </div>
                                ...
                                <div className="col-span-4 w-full panel">
                                  <div className="mb-5">
                                    ...
                                    <input id="search" type="text" placeholder="Recherche produit" className="form-input" value={search} onChange={(e) => setSearch(e.target?.value)} />
                                        {/* <MagnifyingGlassCircleIcon onClick={handleFiltre} style={{cursor:"pointer",marginLeft:-35}} strokeWidth={2} className="h-9 w-9
                                ...
                                <div className="col-span-8  w-full panel">
                                    {panierProduit?.length>0?
                                        panierProduit?.map((el:any,id:number)=>(
                                                 <VenteCard handleValidate={(e:any)=>handleUpdate(e)} key={el.produit_id} handleDelete={()=>handleDelete(el)} data={el} />
                                        ))
                                       :
